...
- 坐标分布测试支持 `dance:fast` 与 `dance:slow`，用于控制普僵系僵尸的 dancecheat 模式

### 1.17.5 - 2026/08/17

- 修复坐标分布测试将死亡动画中的僵尸误计为存活的问题

### 1.17.4 - 2026/07/19

- 修复了 `avzTime` 不修正冰时机的问题（1 冰为完美预判冰，0 冰被忽略）
- `avzTime` 不再修正垫材种植与铲除时机
- 坐标分布与小鬼拦截测试支持使用 `spawnRow` 限定僵尸出怪行

### 1.17.3 - 2026/07/19

- 坐标分布测试支持 `huge:true`，用于模拟旗帜波出生坐标
...
- 增加 `N`、`W`、`W_NUM` 操作
- 增加 `avzTime` 设置：使用 AvZ 的时间基准（预判冰为 1，垫快速为 4n-1）
- 修复波数只能是一位数的 bug
- 增加 `seml.testBlocks` 命令：测试任意文件中的 ` ```seml ``` ` 代码块
- 增加坐标分布测试功能
