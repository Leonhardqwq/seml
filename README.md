...
cobDelay:true          # true 为考虑引信延迟, false 为不考虑 (默认 false)
...
1. 下载本插件.
2. 新建 `.seml` 后缀文件, 使用 `Ctrl + Shift + P` 呼出命令菜单, 键入 `使用坐标分布模板`.
或者手动输入以下内容:
```
# 测试 PE 红眼在 445 垫接预判冰时的坐标分布
scene:PE
...
# protect:18 28 58 68   # 预先生成的植物 (可省略), 此例中为 1,2,5,6 路 7-8 列炮

w 601 900               # 601 冰，记录 900cs 时的数据
C 445+200 1256 9
```

`huge:true` 会按旗帜波生成测试僵尸，使出生坐标在普通波基础上增加 40；该设置同时作用于
指定时刻的坐标分布和 `targetPos` 到达时刻分布。省略时等同于 `huge:false`。

`spawnRow:` 用连续数字指定允许出怪的路，例如 `spawnRow:1` 固定在 1 路，
`spawnRow:1256` 在 1、2、5、6 路中均匀随机。省略时使用模拟器原生选行。

## 快速开始: 小鬼拦截测试

1. 下载本插件.
2. 新建 `.seml` 后缀文件, 使用 `Ctrl + Shift + P` 呼出命令菜单, 键入 `使用小鬼拦截模板`.
或者手动输入以下内容:
```
# 测试弦14炮拦截失败率
scene:PE
...
3. 使用 `Ctrl + Shift + P` 呼出命令菜单, 键入 `测试小鬼拦截` 即可.

## 内联测试

使用 `Ctrl + Shift + P` 呼出命令菜单, 键入 `测试代码块` 即可测试当前文件中所有 ` ```seml ``` ` 代码块. 文件后缀不必为 `.seml`, 代码块以外的内容将被忽略.

代码块的格式为：

````markdown
```seml type=smash name=1
# type 为必选项, 代表测试类型，必须为 smash, explode, refresh, pogo, pos, imp 之一
# name 为可选项, 代表测试名称, 将出现在输出文件名中, 默认为 1, 2, 3, ... 依次递增
[seml 内容]
```
````
...
使用卡片. 基于炮等效时间. 支持樱桃 (A), 辣椒 (J), 毁灭菇 (N), 窝瓜 (a/W), 大蒜 (G).
...
```
scene:PE                # 场地, 六场地大小写皆可
types:白红              # 测试的巨人类型, 可用 白/红 或 garg/giga
impIndex:ratio 0.5      # 小鬼编号模式: native/high/low/ratio, ratio 后可接 0~1 的高编号比例
protect:15 25 55 65     # 要统计啃食伤害的保护植物, 可省略
repeat:10000            # 测试次数 (默认 10000)
```
